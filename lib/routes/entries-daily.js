...
const routesEntries = {
  ...
  register: async function (server, options) {
    ...
    // todo: combine these into a single method that simply overwrites entry
    // see updates-weekly
    server.route({
      method: 'POST',
    ...
    server.route({
      method: 'GET',
      path: '/entries/daily/mood_scores',
      config: {
        auth: 'jwt',
        description: 'Get mood scores within a date range.'
      },
      handler: async (request, h) => {
        const userId = request.auth.credentials.userid;
        const { startDate, endDate } = request.query;
        ...
        try {
          const res = await client.query(`
            SELECT
              date_string, mood_score
            FROM
              daily_entries
            WHERE
              userid = $1
              and date_string between $2 and $3
            ORDER BY
              date_string
          `, [userId, startDate, endDate]);
          return res.rows;
          ...
          console.log(e);
          return Boom.badImplementation('Error getting daily mood scores');
        }
      }
    });
  }
